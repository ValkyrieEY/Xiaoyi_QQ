...
async def handle_timeout(event, actions, Manager, Segments):
    ...
    while get_game_status(gid):
        if datetime.now() < game_session.timeout:
            ...
            await actions.send(group_id=gid, message=Manager.Message(Segments.At(game_session.uid), Segments.Text("由于超时，已为您自动结束游戏")))
            turn_off(gid)
            break
    ...
    if str(event.message) == f"{reminder}猜人":
        ...
        if game_session:
            if uid == game_session.uid:
                ...
                await actions.send(
                    group_id=gid,
                    message=Manager.Message(Segments.Text("您已经开始游戏啦 ~"))
                )
            else:
                await actions.send(
                    group_id=gid,
            ...
            try:
                ...
                if not q:
                    q = "（出错了，未能获取到题目，请稍后再试）"
                turn_on(gid, uid, aki)
                await actions.send(group_id=gid, message=Manager.Message(Segments.Text(
                    f"{q}\n是(y)\n不是(n)\n我不知道(idk)\n或许是(p)\n或许不是(pn)\n上一题(b)\n退出(exit)")))
        ...
        if getattr(aki, "answer_id", None):
            try:
                name = getattr(aki, "name", "未知")
                description = getattr(aki, "description", "")
                photo = getattr(aki, "photo", "")
                msg = f"是 {name} ({description})! 我猜对了么?"
                await actions.send(
                    group_id=event.group_id,
                    message=Manager.Message(
                        Segments.At(event.user_id),
                        Segments.Text(msg),
                        Segments.Image(photo)
                    )
                )
            ...
            game_session.timeout = datetime.now() + timedelta(seconds=60)
            return True