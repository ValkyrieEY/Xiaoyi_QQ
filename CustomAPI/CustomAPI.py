...
# 在初始化 Configurator 后再导入其他 Hyper 模块
...
async def on_message(event, actions):
    ...
    if user_message == "/api config":
        ...
        return Manager.Message(Segments.Text("已打开API配置界面，请在浏览器中操作"))
        ...
        if user_message.startswith(api['command']):
            try:
                ...
                if api['response_type'] == 'image':
                    return Manager.Message(Segments.Image(response_text))
                else:
                    return Manager.Message(Segments.Text(response_text))
            except Exception as e:
                return Manager.Message(Segments.Text(f"API请求失败: {str(e)}"))
