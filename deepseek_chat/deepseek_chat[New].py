...
# 插件元数据
TRIGGHT_KEYWORD = "671b"
HELP_MESSAGE = """DeepSeek AI对话系统
指令：
671b <问题> - 向AI提问
671b 清除记忆 - 清除当前对话历史"""

__name__ = "DeepSeekChat"
__version__ = "1.0.0"
__author__ = "Xiaoyi"
...
async def on_message(event, actions, Manager, Segments):
    ...
    if msg.lower() == "671b 清除记忆":
        ...
        await actions.send(
            ...
            message=Manager.Message(Segments.Text("✅ 已清除对话历史"))
        )
        return True
    ...
    if not question:
        await actions.send(
            group_id=event.group_id,
            message=Manager.Message(Segments.Text("请输入要咨询的问题"))
        )
        return True
    ...
    try:
        ...
        await actions.send(
            group_id=event.group_id,
            message=Manager.Message(Segments.Text(formatted_response))
        )
    except Exception as e:
        await actions.send(
            group_id=event.group_id,
            message=Manager.Message(Segments.Text(f"❌ 记忆库访问失败：{str(e)}"))
        )
