...
async def on_message(event, actions):
    ...
    if message.startswith(spam_system.command_prefix):
        ...
        if cmd == "status":
            ...
            if len(cmd_parts) != 3:
                return Manager.Message(Segments.Text("格式错误！使用方法：!as set <check_time/max_messages> <值>"))
            ...
            try:
                ...
                return Manager.Message(Segments.Text("参数必须是正整数！"))
            ...
            if param == "check_time":
                ...
                return Manager.Message(Segments.Text("未知参数！可用参数：check_time, max_messages"))

            if spam_system.save_config():
                return Manager.Message(Segments.Text(f"设置成功！当前配置：\n检测时间：{spam_system.config['check_time']}秒\n最大消息数：{spam_system.config['max_messages']}条"))
            return Manager.Message(Segments.Text("设置失败！"))

        elif cmd == "toggle":
            spam_system.config["enabled"] = not spam_system.config["enabled"]
            if spam_system.save_config():
                return Manager.Message(Segments.Text(f"系统已{'启用' if spam_system.config['enabled'] else '禁用'}"))
            return Manager.Message(Segments.Text("切换状态失败！"))

        elif cmd == "clear":
            if len(cmd_parts) != 2:
                return Manager.Message(Segments.Text("格式错误！使用方法：!as clear <QQ号>"))
            
            target_id = cmd_parts[1]
            if target_id in spam_system.config["warn_records"]:
                del spam_system.config["warn_records"][target_id]
                if spam_system.save_config():
                    return Manager.Message(Segments.Text(f"已清除 {target_id} 的警告记录"))
            return Manager.Message(Segments.Text("清除失败或用户无警告记录"))
