        ...
        if filename.endswith(".py"):
            ...
            try:
                ...
                if hasattr(module, 'on_message'):
                    ...
                    failed_plugins.append(f"{module_name} (缺少 on_message)")
...
bot_name = Configurator.cm.get_cfg().others["bot_name"] #小依
...
心碎小依是一款基于Jianer_QQ开源版二次开发的QQ机器人，框架基于Lagrange.OneBot和HypeR Bot'''
    ...
    if isinstance(event, Events.GroupMessageEvent):
        ...
        elif user_message == "readplugins":  # 修改这里，只匹配完整的readplugins
            if isinstance(event, Events.GroupMessageEvent):  # 只在群消息事件中处理
                status = f'''{bot_name} {bot_name_en} - 插件系统状态
————————————————————
✅ 已加载插件 ({len(loaded_plugins)}):
{chr(10).join(loaded_plugins) if loaded_plugins else "无"}

❌ 已禁用插件 ({len(disabled_plugins)}):
{chr(10).join(disabled_plugins) if disabled_plugins else "无"}

⚠️ 加载失败 ({len(failed_plugins)}):
{chr(10).join(failed_plugins) if failed_plugins else "无"}'''
...
快来聊天吧(*≧︶≦)'''
            ...
            global version_name
            about = f'''{bot_name} {bot_name_en} - 小依一心为你
————————————————————
Build Information
...
def handle_custom_api(order):
    start_index = order.find("自定义API ")
    if start_index != -1:
        api_command = order[start_index + len("自定义API "):].strip()
        parts = api_command.split()
        if len(parts) < 2:
            return "格式错误。正确格式: 自定义API [API名称] [参数1] [参数2] ..."
        ...
        # 这里可以定义不同的API及其URL
        api_urls = {
            "天气": "http://api.example.com/weather",
            "新闻": "http://api.example.com/news",
            # 添加更多API...
        }

        if api_name not in api_urls:
            return f"未知的API: {api_name}"
        ...
        if result:
            # 这里可以根据不同的API返回格式自定义处理逻辑
            return json.dumps(result, ensure_ascii=False, indent=2)
        else:
            return "API请求失败"
    return None
    
    #API是一个实验功能，因此我们并未启用它，如果您需要，请自行启用，谢谢！
...
if check_website():
    ...
    print("程序启动失败,请检查网站是否正常运行,可能遇到技术问题,请联系管理员QQ2477194503解决,程序将在1秒后退出...")

#感谢您使用心碎小依QQBOT
#最终解释权归心碎小依开发项目组所有
#感谢思锐工作室提供基础源码
#感谢EY Cloud依缘云计算提供服务器支持
#代码中含有前开发团队注释，我们对此表示感谢！
#感谢名单：
#-----------------------------------
#思锐工作室
#心碎小依开发项目组
...
#Xiaoyi  QQ2477194503
#北依辰you
#### 我们的博客：https://xun.eynet.top

#请不要删除以上信息，谢谢！这对我们非常重要！
