<div align="center">
  <img src="https://github.com/user-attachments/assets/4c295469-8cd8-4406-8070-6c096e15a313" alt="Xiaoyi_QQ_Bot Logo" width="250">
  
  # Xiaoyi_Bot_Plugin_marketplace
  
  <p>Xiaoyi Developer Plugin Center</p>

  <p>
    <img src="https://img.shields.io/badge/OneBot-11-black?logo=data:image/png;base64," alt="Badge">
<img src="https://img.shields.io/badge/Language-Python-coral" alt="Language">  
    <img src="https://img.shields.io/github/issues/ValkyrieEY/Xiaoyi_QQ" alt="Issues" />  
    <img src="https://img.shields.io/badge/license-AGPL--3.0-blue" alt="License" />
    <img src="https://img.shields.io/github/stars/ValkyrieEY/Xiaoyi_QQ" alt="Stars" />  
    <img src="https://img.shields.io/github/forks/ValkyrieEY/Xiaoyi_QQ" alt="Forks" />
  </p>
</div>

## ✨ 插件中心

- 🚀 **原生优质** - 通过插件市场可快速完善机器人功能
- 📦 **使用方便** - 模块化插件，仅需拖入即可使用

## 📦 安装


### 快速开始

1. 下载所需插件至本地目录
2. 将插件移动至机器人运行目录 plugins 目录内
3. 重启主要文件，或通过插件管理器热重载插件
4. 检查插件开发文档，配置插件，获取相关指令

### 旧版插件

1. 基于2.0版本的旧版插件不适用于新版本
2. NEXT重构版插件不适用于发行版
3. 可以通过安装插件适配器来使插件通用
4. 请使用old_ plugin_adapter插件正常加载即可

## 🤝 上传插件

1. 联系仓库管理员
2. 发送自己的插件
3. 发送相关信息(版本号/作者/插件名称/介绍)

## 📝 开源协议

本项目基于 [AGPL-3.0](LICENSE) 协议开源。

## 💬 联系我们

- 📧 邮箱：2477194503@qq.com
- 🔔 提交 [Issues](https://github.com/ValkyrieEY/Xiaoyi_QQ/issues)
- 💫 加入 [QQ 交流群](#)

---

<div align="center">

**Xiaoyi_QQ_Bot** ©2024 Created by ValkyrieEY

</div>
