...
from Tools.workflow_manager import WorkflowManager, WorkflowExecutor, WorkflowRoute, WorkflowStep, StepType, RoutingRule
from Tools.group_notice_manager import GroupNoticeManager
# AI插件系统
from ai_plugins import AIPluginManager, BaseAIPlugin
...
workflow_executor = None  # 将在插件加载后初始化

# 群组通知管理器
group_notice_manager = GroupNoticeManager()

# AI插件管理器
ai_plugin_manager = AIPluginManager()
print(f"已加载AI插件: {len(ai_plugin_manager.plugins)} 个")
for plugin_name, plugin in ai_plugin_manager.plugins.items():
    print(f"  - {plugin_name}: {plugin.display_name} ({'enabled' if plugin.is_enabled() else 'disabled'})")
...
# 初始化工作流执行器
workflow_executor = WorkflowExecutor(workflow_manager, plugins)
...
# 工作流消息处理
async def handle_workflow_message(message_context: dict, actions) -> bool:
    """处理工作流消息，返回是否被工作流处理"""
    try:
        # 检查是否启用工作流
        if not workflow_manager.global_config.get("enabled", True):
            return False
        
        # 路由消息到匹配的工作流
        matched_workflows = await workflow_manager.route_message(message_context)
        
        if not matched_workflows:
            return False
        
        # 执行匹配的工作流（优先级高的先执行）
        for workflow_id in matched_workflows:
            try:
                # 注入actions到消息上下文，以便工作流可以发送消息
                message_context['actions'] = actions
                await workflow_executor.execute_workflow(workflow_id, message_context)
                # 如果有工作流处理了消息，返回True
                return True
            except Exception as e:
                print(f"执行工作流 {workflow_id} 时发生错误: {e}")
                continue
        
        return False
        
    except Exception as e:
        print(f"工作流消息处理发生错误: {e}")
        return False
...
async def handler(event: Events.Event, actions: Listener.Actions) -> None:
    ...
    global Super_User, Manage_User, ROOT_User, workflow_executor
...
Welcome! {bot_name} was restarted successfully. Now you can send {reminder}帮助 to know more.''')))
        ...
        # 检查群组是否启用入群欢迎
        if not group_notice_manager.is_welcome_enabled(event.group_id):
            print(f"群组 {event.group_id} 已禁用入群欢迎")
            return
        
        user = event.user_id
        
        # 获取自定义欢迎消息，如果没有则使用默认消息
        custom_welcome = group_notice_manager.get_welcome_message(event.group_id)
        if custom_welcome:
...
祝你在{bot_name}的大家庭里生活愉快！♪(≧∀≦)ゞ☆'''
        ...
        # 检查群组是否启用退群提示
        if not group_notice_manager.is_farewell_enabled(event.group_id):
            print(f"群组 {event.group_id} 已禁用退群提示")
            return
        ...
        # 获取自定义退群消息，如果没有则使用默认消息
        custom_farewell = group_notice_manager.get_farewell_message(event.group_id)
        if custom_farewell:
            text = custom_farewell.replace("{user_nick}", user_nick).replace("{bot_name}", bot_name)
        else:
...
插件 {plugin_name} 已经成功启用''')))
            ...
            plugin = ai_plugin_manager.get_plugin_by_keyword(order)
            if plugin:
                ai_plugin_manager.set_current_plugin(plugin.plugin_name)
                print(f"sys: AI Mode change to {plugin.display_name}")
                
                if plugin.plugin_name == "chatgpt":
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("嗯……我好像升级了！o((>ω< ))o")))
                elif plugin.plugin_name == "deepseek":
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("服务器……繁忙？ε٩(๑> ₃ <)۶з")))
                elif plugin.plugin_name == "gemini":
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"{bot_name}打开了新视界！o(*≧▽≦)ツ")))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"已切换到 {plugin.display_name} 模式！")))
                return 
            
        if order in ["默认4", "默认3.5", "chatgpt", "gpt", "Net", "Normal"]:
            if ai_plugin_manager.set_current_plugin("chatgpt"):
                print(f"sys: AI Mode change to ChatGPT")
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("嗯……我好像升级了！o((>ω< ))o")))
            else:
                EnableNetwork = "Net"
        elif order in ["深度", "deepseek", "Ds"]:

            if ai_plugin_manager.set_current_plugin("deepseek"):
                print(f"sys: AI Mode change to DeepSeek")
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("服务器……繁忙？ε٩(๑> ₃ <)۶з")))
            else:
                EnableNetwork = "Ds"
        elif order in ["读图", "gemini", "Pixmap", "图片"]:
            if ai_plugin_manager.set_current_plugin("gemini"):
                print(f"sys: AI Mode change to Gemini")
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"{bot_name}打开了新视界！o(*≧▽≦)ツ")))
            else:
                EnableNetwork = "Pixmap"  # 兼容模式
        
        elif "全局记忆" == order:
            current_plugin = ai_plugin_manager.get_current_plugin()
            if current_plugin:
                from ai_plugins.memory_manager import MemoryMode
                current_plugin.set_memory_mode(str(event.user_id), MemoryMode.GLOBAL)
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"已切换到全局记忆模式！AI将记住群里所有人的对话内容。")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前没有激活AI插件，无法切换记忆模式。")))
        
        elif "个人记忆" == order:
            current_plugin = ai_plugin_manager.get_current_plugin()
            if current_plugin:
                from ai_plugins.memory_manager import MemoryMode
                current_plugin.set_memory_mode(str(event.user_id), MemoryMode.PERSONAL)
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"已切换到个人记忆模式！AI将只记住你的对话内容。")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前没有激活AI插件，无法切换记忆模式。")))
            ...
            current_plugin = ai_plugin_manager.get_current_plugin()
            if current_plugin:
                status = current_plugin.get_memory_status(str(event.user_id), str(event.group_id))
                global_preset = current_plugin.get_global_preset(str(event.group_id))
                
                status_text = f"""当前AI记忆状态：
记忆模式：{status['mode_description']}
上下文ID：{status['context_id']}
消息数量：{status['message_count']} 条"""
                ...
                status_text += f"""\n\n操作指令：
{reminder}全局记忆 - 切换到群组共享记忆
{reminder}个人记忆 - 切换到个人独立记忆
{reminder}清除记忆 - 清除当前记忆模式下的所有对话"""
                
                if str(event.user_id) in ADMINS:
                    status_text += f"""\n\n管理员指令：
{reminder}设置全局预设 [预设名称] - 设置群组全局预设
{reminder}查看全局预设 - 查看当前群组全局预设
{reminder}清除全局预设 - 清除群组全局预设"""
                
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(status_text)))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前没有激活AI插件，无法查看记忆状态。")))
        
        elif "清除记忆" == order:
            current_plugin = ai_plugin_manager.get_current_plugin()
            if current_plugin:
                # 获取用户的记忆模式
                memory_mode = current_plugin.get_memory_mode(str(event.user_id))
                
                if memory_mode.value == "global":
                    if str(event.user_id) in ADMINS:
                        current_plugin.clear_memory(str(event.user_id), str(event.group_id))
                        await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("已清除群组全局记忆！所有群友的对话历史已被清除。")))
                        # 通知ROOT用户
                        r_admin = f'''用户 {await get_user_nickname(event.user_id, Manager, actions)} 在 {event.time_str} 清除了群 {event.group_id} 的全局AI记忆'''
                        await actions.send(user_id=ROOT_User[0], message=Manager.Message(Segments.Text(r_admin)))
                    else:
                        await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("全局记忆清除需要管理员权限！您可以切换到个人记忆模式后清除自己的记忆。")))
                else:
                    current_plugin.clear_memory(str(event.user_id), str(event.group_id))
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("已清除您的个人记忆！您与机器人的对话历史已被清除。")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前没有激活AI插件，无法清除记忆。")))
        
        # 全局预设管理指令
        elif "设置全局预设 " in order:
            if str(event.user_id) in ADMINS:
                current_plugin = ai_plugin_manager.get_current_plugin()
                if current_plugin:
                    preset_name = order[order.find("设置全局预设 ") + len("设置全局预设 "):].strip()
                    if preset_name:
                        presets = presets_tool.read_presets()
                        preset_exists = False
                        ...
                        if preset_exists:
                            current_plugin.set_global_preset(str(event.group_id), preset_name)
                            await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"已设置群组全局预设为：{preset_name}\n\n此预设将用于全局记忆模式下的所有AI对话。")))
                            # 通知ROOT用户
                            r_admin = f'''用户 {await get_user_nickname(event.user_id, Manager, actions)} 在 {event.time_str} 将群 {event.group_id} 的全局预设设置为：{preset_name}'''
                            await actions.send(user_id=ROOT_User[0], message=Manager.Message(Segments.Text(r_admin)))
                        else:
                            await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"预设 '{preset_name}' 不存在！请先创建该预设或使用现有预设名称。")))
                    else:
                        await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"请指定预设名称！\n\n用法：{reminder}设置全局预设 [预设名称]")))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前没有激活AI插件，无法设置全局预设。")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
            ...
            if current_plugin:
                global_preset = current_plugin.get_global_preset(str(event.group_id))
                if global_preset:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"当前群组全局预设：{global_preset}\n\n该预设用于全局记忆模式下的所有AI对话。")))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前群组未设置全局预设。\n\n全局记忆模式下将使用用户个人预设。")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前没有激活AI插件，无法查看全局预设。")))
        
        elif "清除全局预设" == order:
            if str(event.user_id) in ADMINS:
                current_plugin = ai_plugin_manager.get_current_plugin()
                if current_plugin:
                    current_plugin.clear_global_preset(str(event.group_id))
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("已清除群组全局预设！\n\n全局记忆模式下将使用用户个人预设。")))
                    # 通知ROOT用户
                    r_admin = f'''用户 {await get_user_nickname(event.user_id, Manager, actions)} 在 {event.time_str} 清除了群 {event.group_id} 的全局预设'''
                    await actions.send(user_id=ROOT_User[0], message=Manager.Message(Segments.Text(r_admin)))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("当前没有激活AI插件，无法清除全局预设。")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
...
if failed_plugins else "无"}'''
            ...
            if str(event.user_id) in ADMINS:
                content = [
                    ...
                    (f"{reminder}工作流列表", "查看所有工作流"),
                    (f"{reminder}工作流状态", "查看工作流系统状态"),
                    (f"{reminder}工作流开关", "切换工作流系统开关"),
                    (f"{reminder}工作流重载", "重新加载所有工作流"),
                    (f"{reminder}群组通知", "管理入群和退群提示开关"),
                    (f"{reminder}入群欢迎 开/关", "切换当前群的入群欢迎功能"),
                    (f"{reminder}退群提示 开/关", "切换当前群的退群提示功能"),
                    (f"{reminder}设置入群消息 (内容)", "设置当前群的自定义入群欢迎消息"),
                    (f"{reminder}设置退群消息 (内容)", "设置当前群的自定义退群提示消息")
                ]
...
“{word}”'''
        ...
        # 工作流管理命令
        elif f"{reminder}工作流列表" == user_message:
            workflows = workflow_manager.list_workflows()
            if not workflows:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("暂时没有可用的工作流。")))
            else:
                workflow_list = f"{bot_name} {bot_name_en} - 工作流列表\n————————————————————\n"
                for i, wf in enumerate(workflows, 1):
                    status = "✅" if wf.enabled else "❌"
                    workflow_list += f"{i}. {status} {wf.name}\n   ID: {wf.id}\n   描述: {wf.description}\n\n"
                workflow_list += f"使用 {reminder}工作流详情 <ID> 查看具体信息"
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(workflow_list)))
        
        elif f"{reminder}工作流详情 " in user_message:
            workflow_id = user_message[user_message.find(f"{reminder}工作流详情 ") + len(f"{reminder}工作流详情 "):].strip()
            workflow = workflow_manager.get_workflow(workflow_id)
            if not workflow:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"未找到工作流: {workflow_id}")))
            else:
                detail = f"{bot_name} {bot_name_en} - 工作流详情\n————————————————————\n"
                detail += f"名称: {workflow.name}\n"
                detail += f"ID: {workflow.id}\n"
                detail += f"状态: {'\u2705 已启用' if workflow.enabled else '\u274c 已禁用'}\n"
                detail += f"描述: {workflow.description}\n\n"
                
                detail += f"路由规则 ({len(workflow.routes)}):\n"
                for i, route in enumerate(workflow.routes, 1):
                    status = "✅" if route.enabled else "❌"
                    detail += f"{i}. {status} {route.name} ({route.rule_type.value})\n"
                    detail += f"   条件: {route.condition}\n"
                    detail += f"   优先级: {route.priority}\n\n"
                
                detail += f"执行步骤 ({len(workflow.steps)}):\n"
                for i, step in enumerate(workflow.steps, 1):
                    status = "✅" if step.enabled else "❌"
                    detail += f"{i}. {status} {step.name} ({step.type.value})\n"
                    if step.description:
                        detail += f"   描述: {step.description}\n"
                
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(detail)))
        
        elif f"{reminder}工作流状态" == user_message:
            if str(event.user_id) in ADMINS:
                config_status = "✅ 已启用" if workflow_manager.global_config.get("enabled", True) else "❌ 已禁用"
                total_workflows = len(workflow_manager.workflows)
                enabled_workflows = len([wf for wf in workflow_manager.workflows.values() if wf.enabled])
                
                status_msg = f"{bot_name} {bot_name_en} - 工作流系统状态\n————————————————————\n"
                status_msg += f"系统状态: {config_status}\n"
                status_msg += f"工作流总数: {total_workflows}\n"
                status_msg += f"已启用: {enabled_workflows}\n"
                status_msg += f"已禁用: {total_workflows - enabled_workflows}\n\n"
                
                if workflow_executor.active_executions:
                    status_msg += f"正在执行: {len(workflow_executor.active_executions)} 个工作流\n"
                    for exec_id, ctx in workflow_executor.active_executions.items():
                        elapsed = (datetime.now() - ctx.start_time).seconds
                        status_msg += f"  - {ctx.workflow_id} (执行 {elapsed}秒)\n"
                else:
                    status_msg += "暂无正在执行的工作流\n"
                
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(status_msg)))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
        
        elif f"{reminder}工作流开关" == user_message:
            if str(event.user_id) in ADMINS:
                current_status = workflow_manager.global_config.get("enabled", True)
                new_status = not current_status
                workflow_manager.global_config["enabled"] = new_status
                workflow_manager.save_config()
                
                status_text = "已启用" if new_status else "已禁用"
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"工作流系统{status_text}！")))
                
                r_admin = f"用户 {await get_user_nickname(event.user_id, Manager, actions)} 在 {event.time_str} {status_text}了工作流系统"
                await actions.send(user_id=ROOT_User[0], message=Manager.Message(Segments.Text(r_admin)))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
        
        elif f"{reminder}工作流重载" == user_message:
            if str(event.user_id) in ADMINS:
                try:
                    workflow_manager.load_workflows()
                    workflow_executor = WorkflowExecutor(workflow_manager, plugins)
                    
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text("工作流重载成功！")))
                    
                    r_admin = f"用户 {await get_user_nickname(event.user_id, Manager, actions)} 在 {event.time_str} 重载了工作流系统"
                    await actions.send(user_id=ROOT_User[0], message=Manager.Message(Segments.Text(r_admin)))
                except Exception as e:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"工作流重载失败: {e}")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
                
        # 群组通知管理命令
        elif f"{reminder}群组通知" == user_message:
            if str(event.user_id) in ADMINS:
                status = group_notice_manager.get_group_status(event.group_id)
                welcome_status = "✅ 已启用" if status['welcome_enabled'] else "❌ 已禁用"
                farewell_status = "✅ 已启用" if status['farewell_enabled'] else "❌ 已禁用"
                custom_welcome = "✅ 已设置" if status['has_custom_welcome'] else "❌ 未设置"
                custom_farewell = "✅ 已设置" if status['has_custom_farewell'] else "❌ 未设置"
                
                notice_info = f"""{bot_name} {bot_name_en} - 群组通知管理
————————————————————
当前群组: {event.group_id}
入群欢迎: {welcome_status}
退群提示: {farewell_status}
自定义入群消息: {custom_welcome}
自定义退群消息: {custom_farewell}
————————————————————
管理命令:
{reminder}入群欢迎 开/关 - 切换入群欢迎功能
{reminder}退群提示 开/关 - 切换退群提示功能
{reminder}设置入群消息 (内容) - 设置自定义入群欢迎消息
{reminder}设置退群消息 (内容) - 设置自定义退群提示消息"""
                
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(notice_info)))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
        
        elif "入群欢迎 " in order:
            if str(event.user_id) in ADMINS:
                action = order.split("入群欢迎 ")[1].strip()
                if action == "开":
                    group_notice_manager.set_welcome_enabled(event.group_id, True)
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"群组 {event.group_id} 的入群欢迎功能已启用！")))
                elif action == "关":
                    group_notice_manager.set_welcome_enabled(event.group_id, False)
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"群组 {event.group_id} 的入群欢迎功能已禁用！")))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"格式错误，请使用：{reminder}入群欢迎 开 或 {reminder}入群欢迎 关")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
        
        elif "退群提示 " in order:
            if str(event.user_id) in ADMINS:
                action = order.split("退群提示 ")[1].strip()
                if action == "开":
                    group_notice_manager.set_farewell_enabled(event.group_id, True)
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"群组 {event.group_id} 的退群提示功能已启用！")))
                elif action == "关":
                    group_notice_manager.set_farewell_enabled(event.group_id, False)
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"群组 {event.group_id} 的退群提示功能已禁用！")))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"格式错误，请使用：{reminder}退群提示 开 或 {reminder}退群提示 关")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
        
        elif "设置入群消息 " in order:
            if str(event.user_id) in ADMINS:
                message_content = order.split("设置入群消息 ", 1)[1].strip()
                if message_content:
                    group_notice_manager.set_welcome_message(event.group_id, message_content)
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"群组 {event.group_id} 的入群欢迎消息已设置！")))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"格式错误，请使用：{reminder}设置入群消息 (消息内容)")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
        
        elif "设置退群消息 " in order:
            if str(event.user_id) in ADMINS:
                message_content = order.split("设置退群消息 ", 1)[1].strip()
                if message_content:
                    group_notice_manager.set_farewell_message(event.group_id, message_content)
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"群组 {event.group_id} 的退群提示消息已设置！")))
                else:
                    await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(f"格式错误，请使用：{reminder}设置退群消息 (消息内容)")))
            else:
                await actions.send(group_id=event.group_id, message=Manager.Message(Segments.Text(CONFUSED_WORD.format(bot_name=bot_name))))
        ...
        #         await actions.set_group_ban(group_id=event.group_id,user_id=event.user_id,duration=600)
            ...
            # 确定用户权限
            if str(event.user_id) in ROOT_User:
                message_context['user_permissions'].append('ROOT_User')
                message_context['user_permissions'].append('ADMINS')
            elif str(event.user_id) in Super_User:
                message_context['user_permissions'].append('Super_User')
                message_context['user_permissions'].append('ADMINS')
            elif str(event.user_id) in Manage_User:
                message_context['user_permissions'].append('Manage_User')
                message_context['user_permissions'].append('ADMINS')
            
            # 尝试工作流处理
            try:
                if await handle_workflow_message(message_context, actions):
                    return  # 工作流处理了消息，不再继续后续处理
            except Exception as e:
                print(f"工作流处理消息时发生错误: {e}")
            ...
            async def handle_message_stream_plugin(response_stream):
                """处理AI插件的流式响应"""
                nonlocal result, sended, enable_forward_msg_num
                async for partial in response_stream:
            ...
            try:
                # 优先使用AI插件系统
                current_plugin = ai_plugin_manager.get_current_plugin()
                if current_plugin and current_plugin.is_enabled():
                    print(f"Using AI plugin: {current_plugin.display_name}")
                    
                    # 根据插件类型处理消息
                    if current_plugin.plugin_name == "gemini":
                        # Gemini插件支持图片处理
                        ...
                        # 调用插件
                        response_stream = current_plugin.process_message(
                            message=msg,
                        ...
                        await handle_message_stream_plugin(response_stream)
...
def help_message() -> str:
    global bot_name, reminder, plugins_help
    
    # 获取当前AI插件信息
    current_plugin = ai_plugin_manager.get_current_plugin()
    current_ai_info = f"（当前：{current_plugin.display_name}）" if current_plugin else ""
    ...
    for plugin_name, plugin in ai_plugin_manager.plugins.items():
        if plugin.is_enabled():
            for keyword in plugin.trigger_keywords:
                current_mark = "（当前）" if plugin == current_plugin else ""
                ai_plugins_help += f"\n       {reminder}{keyword}{current_mark} —> {plugin.display_name}"
    
    return f'''如何与{bot_name}交流( •̀ ω •́ )✧
    注：对话前必须加上 {reminder} 噢！~
       {reminder}(任意问题，必填) —> {bot_name}回复{current_ai_info}{ai_plugins_help}{plugins_help}
       {reminder}记忆状态 —> 查看当前AI记忆模式和状态
       {reminder}全局记忆 —> 切换到群组共享记忆模式
       {reminder}个人记忆 —> 切换到个人独立记忆模式
       {reminder}清除记忆 —> 清除当前记忆模式下的所有对话
...
if __name__ == "__main__":
    print(f"{bot_name} {bot_name_en} - {ONE_SLOGAN}")
    print(f"正在启动 {bot_name}...")
    print(f"版本: {version_name}")
    print("\n" + "="*50)
    
    try:
        print("正在连接到OneBot服务端...")
        import json
        with open('config.json', 'r', encoding='utf-8') as f:
            config_data = json.load(f)
        connection_info = config_data.get('Connection', {})
        print(f"连接地址: {connection_info.get('host', '127.0.0.1')}:{connection_info.get('port', 5004)}")
        print(f"监听地址: {connection_info.get('listener_host', '127.0.0.1')}:{connection_info.get('listener_port', 5003)}")
        ...
        print(f"\n启动失败: {e}")
        print("请检查OneBot服务端是否正常运行")
        print("或检查config.json中的连接配置")
        print("\n如果没有OneBot服务端，请先启动一个（如 go-cqhttp）")
else:
    # 当作为模块导入时，不自动启动
    print(f"{bot_name} 模块已加载，使用 Listener.run() 手动启动")