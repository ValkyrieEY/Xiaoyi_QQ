...
async def on_message(event, actions):
    ...
    # 管理员命令
    if not is_admin:
        return Manager.Message(Segments.Text("你没有权限执行此操作"))

    if cmd == "toggle":
        enabled = not blacklist_system.admins["enabled"]
        if blacklist_system.toggle_system(enabled):
            return Manager.Message(Segments.Text(f"黑名单系统已{'启用' if enabled else '禁用'}"))
        return Manager.Message(Segments.Text("切换系统状态失败"))

    # 如果系统未启用，其他管理员命令不可用
    if not blacklist_system.is_system_enabled():
        return Manager.Message(Segments.Text("黑名单系统当前已禁用"))
    ...
    if cmd == "add" and len(cmd_parts) >= 3:
        ...
        if blacklist_system.add_to_blacklist(target_id, reason, user_id):
            return Manager.Message(Segments.Text(f"已将用户 {target_id} 添加至黑名单\n原因：{reason}"))
        return Manager.Message(Segments.Text("添加黑名单失败"))

    elif cmd == "remove" and len(cmd_parts) == 2:
        target_id = cmd_parts[1]
        if blacklist_system.remove_from_blacklist(target_id):
            return Manager.Message(Segments.Text(f"已将用户 {target_id} 从黑名单中移除"))
        return Manager.Message(Segments.Text("移除黑名单失败"))

    elif cmd == "addadmin" and len(cmd_parts) == 2:
        target_id = cmd_parts[1]
        if blacklist_system.add_admin(target_id):
            return Manager.Message(Segments.Text(f"已将用户 {target_id} 添加为管理员"))
        return Manager.Message(Segments.Text("添加管理员失败"))

    elif cmd == "removeadmin" and len(cmd_parts) == 2:
        target_id = cmd_parts[1]
        if blacklist_system.remove_admin(target_id):
            return Manager.Message(Segments.Text(f"已将用户 {target_id} 移除管理员权限"))
        return Manager.Message(Segments.Text("移除管理员失败"))
